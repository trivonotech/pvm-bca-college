...
export function Hero() {
    ...
    return (
        <section className="relative w-full min-h-0 lg:min-h-[90vh] bg-[#FDFDFF] overflow-hidden flex flex-col items-center font-sans pt-8 lg:pt-16 mb-6 lg:mb-[46px]">
            {/* Background Grid */}
            ...
            {/* Main Content Area */}
            <div className="container mx-auto px-4 relative z-10 lg:flex-grow flex flex-col lg:flex-row items-center lg:items-center lg:justify-between pointer-events-none">

                {/* Left: Illustration (Boy) */}
                <div className="w-full lg:w-1/2 flex justify-center lg:justify-start pointer-events-auto mt-4 lg:mt-0">
                    <img
                        src={content?.images?.hero || heroIllustration}
                        alt="A student successfully graduating from PVM BCA College - Best BCA Education"
                        className="w-full max-w-[280px] md:max-w-xs lg:max-w-xl object-contain transform lg:translate-x-12"
                    />
                </div>

                {/* 'Start Your Journey' Button - Relative on mobile, Absolute on Desktop */}
                <div className="relative w-full flex justify-center mt-2 lg:mt-0 lg:absolute lg:top-[35%] lg:right-24 z-30 pointer-events-auto lg:transform lg:-translate-y-1/2 lg:justify-end lg:w-auto lg:pr-0">
                    <Link
